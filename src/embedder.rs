...
/// Object-safe embedding trait. Returns [`EMBEDDING_DIMS`]-dimensional f32 vectors.
    ...
    async fn post_with_retry(
        ...
        let result = tokio::time::timeout(TOTAL_RETRY_TIMEOUT, async {
            ...
            for attempt in 0..=MAX_RETRIES {
                ...
                if is_retryable(status) {
                    ...
                    let safe_body = crate::redact::redact_tokens(&resp_body);
        ...
        match result {
            ...
            Err(_) => Err(EmbedError::Api {
                status: 0,
                message: "Gemini API did not respond within retry window (180s)".into(),
            }),
        }
    }
}
...
fn validate_dims(values: &[f32]) -> Result<(), EmbedError> {
...
impl Embed for FailingEmbedder {
    ...
    fn embed_documents<'a>(&'a self, _texts: &'a [String]) -> EmbedFuture<'a, Vec<Vec<f32>>> {
        Box::pin(async { Err(EmbedError::Api { status: 503, message: "service unavailable".into() }) })
    }
}
...
mod tests {
    ...
    fn from_env_returns_error_without_api_key() {
        ...
        assert!(result.unwrap_err().to_string().contains("GEMINI_API_KEY not set"));
    }
    ...
    fn parse_single_embedding_ok() {
        ...
        let json = serde_json::json!({ "embedding": { "values": values } });
    ...
    fn parse_batch_embeddings_rejects_wrong_dims() {
        let json = serde_json::json!({ "embeddings": [{ "values": [0.1, 0.2] }] });
        assert!(parse_batch_embeddings(&json).is_err());
    }

    #[test]
    fn parse_single_embedding_rejects_non_numeric() {
        let json = serde_json::json!({ "embedding": { "values": [0.1, "bad", 0.3] } });
        assert!(parse_single_embedding(&json).is_err());
    }
    ...
    async fn post_with_retry_retries_on_429() {
        ...
        Mock::given(method("POST"))
            .respond_with(ResponseTemplate::new(200).set_body_json(serde_json::json!({
                "embedding": { "values": vec![0.1_f32; 768] }
            })))
            .mount(&server)
            .await;
    ...
    async fn post_with_retry_aborts_on_non_retryable_error() {
        ...
        let embedder = Embedder::with_base_url(Client::new(), "test-key".into(), server.uri());
        let err = embedder.embed_query("test").await.unwrap_err();
